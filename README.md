...
[JSON](config/datasources.json.example) and [YAML](config/datasource.yaml.example). The formal
contract is the [JSON Schema](config/sqlconduit.schema.json); the JSON example associates it through
`$schema`, while the YAML example uses a `yaml-language-server` directive to share the same schema,
providing IDE field, type, enum, range, and default-value hints. Unknown fields, wrong types,
and out-of-range values fail instead of being ignored or normalized. File and semantic failures
return `ConfigError`, missing client driver registration returns `UnknownDriver`, and connection
establishment failures return `ConnectionFailed`. Prefer `password_env` in production instead of
storing a password in the file.
...
`lib/`, `lib/cmake/sqlconduit/`, and `lib/pkgconfig/`. Formal releases also publish `SHA256SUMS`,
an SPDX JSON SBOM, and build provenance verifiable with `gh attestation verify`.
...
See the [metrics contract](docs/metrics.md) for compatibility and cardinality rules and the
[benchmark guide](benchmarks/README.md) for performance measurements. The [changelog](CHANGELOG.md)
contains release highlights. Contribution, vulnerability-reporting, and version-support policies
are in [CONTRIBUTING](CONTRIBUTING.md), [SECURITY](SECURITY.md), and [SUPPORT](SUPPORT.md).
