...
namespace sqlconduit
{
    ...
    EntityResult<T> queryAs(const std::string& sql)
    {
        common::ResultSet rs;
        EntityResult<T> r;
        r.status = SQLConduit::query(sql, rs);
    ...
    EntityResult<T> queryAs(const std::string& sql, const common::Params& params)
    {
        common::ResultSet rs;
        EntityResult<T> r;
        r.status = SQLConduit::query(sql, params, rs);
        if (r.status.ok()) r.status = mapping::fromRows<T>(rs, r.items);
        return r;
    }
                            ...
                            const common::Params& params)
    {
        common::ResultSet rs;
        EntityResult<T> r;
        r.status = SQLConduit::query(dataSource, sql, params, rs);
        if (r.status.ok()) r.status = mapping::fromRows<T>(rs, r.items);
        return r;
    }
                               ...
                               const std::function<bool(T&&)>& cb, std::uint64_t& rows)
    {
        ...
        const auto st = SQLConduit::queryEach(sql, params, raw, driverRows);
        rows = counted;
                               ...
                               const std::function<bool(T&&)>& cb, std::uint64_t& rows)
    {
        ...
        std::uint64_t driverRows = 0;
        const auto st = SQLConduit::queryEach(dataSource, sql, params, raw, driverRows);
        rows = counted;
        if (st.ok() && !mapErr.ok()) return mapErr;
        return st;
    }
    ...
    WriteResult<T> insertAs(std::string table, T& entity)
    {
        ...
        r.status = SQLConduit::withSession([&](core::Session& s)
        {
    ...
    WriteResult<T> updateAs(std::string table, const T& entity)
    {
        ...
        r.status = SQLConduit::execute(sql, mapping::updateParamsOf(entity), r.affected);
        return r;
    }
    ...
    BatchWriteResult<T> insertBatchAs(std::string table, const std::vector<T>& entities)
    {
        ...
        r.status = SQLConduit::executeBatch(mapping::insertSql<T>(table, mapping::defaultDialect()),
    ...
    {
        BatchWriteResult<T> r;
        r.status = SQLConduit::executeBatch(
