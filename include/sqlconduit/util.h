...
namespace sqlconduit::common::util
{
    ...
    {
        const auto ds = SQLConduit::dataSource(dataSource);
                               ...
                               const Dialect d, const bool returnsRows, CallPlan& out)
    {
        ...
        if (d == Dialect::SqlServer && hasOut)
            return unsupported("sqlconduit::util: sqlserver OUT/INOUT parameters need DECLARE @var "
                "<type> before EXEC ... OUTPUT, and SQLConduit cannot infer the type; return the "
        ...
        if (d == Dialect::Oracle && hasOut)
            return unsupported("sqlconduit::util: Oracle output binds are executed through "
                "SQLConduit::call()/util::call(), not represented by CallPlan");
        if (d == Dialect::Oracle && ref.kind == RoutineKind::Procedure && returnsRows)
            return unsupported("sqlconduit::util: Oracle procedure result sets require a REF CURSOR "
                "CallParam and SQLConduit::call()/util::call()");
        ...
        inline Status runDdl(const ExecOptions& o, const std::string& sql)
        {
            ...
            Status st;
            {
                ...
                st = o.dataSource.empty()
                         ? SQLConduit::execute(sql, affected)
                         : SQLConduit::execute(o.dataSource, sql, affected);
            }
            return st;
        }
                       ...
                       std::int64_t& affected, const CallOptions& opts = {})
    {
        ...
        return opts.dataSource.empty()
                   ? SQLConduit::execute(sql, params, affected)
                   : SQLConduit::execute(opts.dataSource, sql, params, affected);
    }
                            ...
                            ResultSet& out, const CallOptions& opts = {})
    {
        const detail::ExecScope scope(opts);
        return opts.dataSource.empty()
                   ? SQLConduit::query(sql, params, out)
                   : SQLConduit::query(opts.dataSource, sql, params, out);
    }
                           ...
                           const CallOptions& opts = {})
    {
        ...
        const detail::ExecScope scope(opts);
        return opts.dataSource.empty()
                   ? SQLConduit::queryEach(sql, params, cb, rows)
                   : SQLConduit::queryEach(opts.dataSource, sql, params, cb, rows);
    }
                       ...
                       const CallOptions& opts = {})
    {
        ...
        if (d == Dialect::Oracle && ref.kind == RoutineKind::Procedure)
        {
            ...
            out.status = o.dataSource.empty()
                             ? SQLConduit::call(sql, params, native)
                             : SQLConduit::call(o.dataSource, sql, params, native);
        ...
        if (o.returnsRows)
        {
            std::vector<ResultSet> sets;
            out.status = o.dataSource.empty()
                             ? SQLConduit::queryAll(plan.callSql, plan.callParams, sets)
                             : SQLConduit::queryAll(o.dataSource, plan.callSql, plan.callParams, sets);
            out.sets = std::move(sets);
        }
        else
        {
            out.status = o.dataSource.empty()
                             ? SQLConduit::execute(plan.callSql, plan.callParams, out.affected)
                             : SQLConduit::execute(o.dataSource, plan.callSql, plan.callParams,
                                                   out.affected);
        }
...
namespace sqlconduit::async::util
{
        ...
        if (plan.needsSameConnection || plan.outFromRowCount > 0)
        {
            if (cb)
            {
                ...
                r.status = common::util::unsupported(
                    "sqlconduit::util: OUT/INOUT parameters are not supported on the async path; "
    ...
    inline Handle createRoutine(std::string sql, OpCallback cb, Options opts = {})
    {
        if (sql.empty())
        {
            detail::failOp(cb, common::util::badSpec("sqlconduit::util: createRoutine sql is empty"));
            return {};
        }
        const auto d = opts.dialect != common::util::Dialect::Auto
                           ? opts.dialect
                              ...
                              Options opts = {})
    {
        ...
        if (spec.concurrent && core::currentTransactionDepth() > 0)
        {
            detail::failOp(cb, common::Status::error(
                               ...
                               "sqlconduit::util: CREATE INDEX CONCURRENTLY cannot run inside a transaction block"));
            return Handle();
        }
        const common::util::detail::ExecScope scope(detail::toExec(opts));
        return opts.dataSource.empty()
    ...
    inline Handle createIndexSql(std::string sql, OpCallback cb, Options opts = {})
    {
        if (sql.empty())
        {
            detail::failOp(cb, common::util::badSpec("sqlconduit::util: createIndexSql sql is empty"));
            return Handle();
        }
        const common::util::detail::ExecScope scope(detail::toExec(opts));
        return opts.dataSource.empty()
    ...
    namespace detail
    {
        ...
        inline void scriptRunFile(std::shared_ptr<ScriptRunState> st)
        {
            ...
            if (!common::util::readSqlFile(path, content))
            {
                common::util::ScriptResult fr;
                fr.path = path;
                fr.status = common::Status::error(common::ErrorCode::IoError,
                                                  "sqlconduit::util: cannot read file: " + path);
                                  ...
                                  common::util::ScriptOptions opts = {})
    {
        ...
        if (!std::filesystem::exists(dir, ec))
        {
            ...
            r.status = common::Status::error(common::ErrorCode::IoError,
                                             "sqlconduit::util: directory not found: " + dir);
            if (cb) cb(std::move(r));
            return Handle();
        }
        std::vector<std::string> files;
