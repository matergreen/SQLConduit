...
int main()
{
    g_configPath = (std::filesystem::temp_directory_path() /
        "sqlconduit_async_test.json").string();
    ...
    std::cout << "== A5. 缓存：命中零驱动调用，写失效（T6 + T12 缓存交互）==\n";
    {
        ...
        f.cache = true;
        applyConfig(f);
        check(SQLConduit::reload(g_configPath, std::chrono::milliseconds(500)).ok(),
              "热加载开启查询缓存");
        ...
        check(SQLConduit::query("SELECT cacheable", syncRows).ok() &&
              AsyncMockConnection::queryCalls == 0 &&
              syncRows.rowCount() == 1,
              "同步 query 命中异步写入的缓存（跨路径共享）");

        std::int64_t affected = 0;
        check(SQLConduit::execute("UPDATE t SET v = 1", affected).ok(),
              "同步写执行成功");
        AsyncMockConnection::queryCalls = 0;
    ...
    std::cout << "== A8. 一致性矩阵：同步 vs 异步逐项相等（T12/R1）==\n";
    {
              ...
              "热加载场景 1 配置（无熔断）");
        {
            ...
            AsyncMockConnection::queryFailuresRemaining = 2;
            common::ResultSet rs;
            syncCode = SQLConduit::query("SELECT m1", rs).code;
              ...
              "热加载场景 2 配置（熔断阈值 2）");
        {
            ...
            AsyncMockConnection::queryFailuresRemaining = 2;
            common::ResultSet rs;
            const auto first = SQLConduit::query("SELECT m2", rs);
            const auto second = SQLConduit::query("SELECT m2", rs);
...

        {
            ...
            AsyncMockConnection::executeCalls = 0;
            std::int64_t n = 0;
            const auto s = SQLConduit::execute("UPDATE t SET v = 1", n);
            syncCode = s.code;
    ...
    std::cout << "== A10. 生命周期：shutdown 排水，之后新操作被拒（T11/§9.3）==\n";
    {
        ...
        const auto t0 = std::chrono::steady_clock::now();
        SQLConduit::shutdown(std::chrono::milliseconds(3000));
        const auto ms = std::chrono::duration_cast<std::chrono::milliseconds>(
            std::chrono::steady_clock::now() - t0).count();
