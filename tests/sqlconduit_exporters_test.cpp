...
int main()
{
    ...
    std::cout << "== M3 指标导出：Prometheus 文本格式 — 池指标 ==\n";
    {
        ...
        check(contains(text, "# HELP sqlconduit_pool_connections "),
              "sqlconduit_pool_connections 含 HELP 行");
        check(contains(text, "# TYPE sqlconduit_pool_connections gauge"),
              "sqlconduit_pool_connections 类型为 gauge");
        check(contains(text, "# HELP sqlconduit_pool_borrow_timeouts_total "),
              "borrow_timeouts 含 HELP 行");
        check(contains(text, "# TYPE sqlconduit_pool_borrow_timeouts_total counter"),
              "borrow_timeouts 类型为 counter");
        check(contains(text, "# HELP sqlconduit_pool_utilization_ratio "),
        ...
        check(contains(text, R"(sqlconduit_pool_utilization_ratio{data_source="ds-app"} 0.7)"),
              "ds-app utilization = 0.7");
        check(contains(text, R"(sqlconduit_pool_utilization_ratio{data_source="ds-rep"} 0)"),
              "ds-rep utilization = 0（无借用）");

        check(contains(text, R"(sqlconduit_pool_borrow_requests_total{data_source="ds-rep"} 100)"),
              "ds-rep borrow_requests_total = 100");
        check(contains(text, R"(sqlconduit_pool_borrow_timeouts_total{data_source="ds-rep"} 2)"),
              "ds-rep borrow_timeouts_total = 2");

        check(contains(text, R"(sqlconduit_pool_waiting{data_source="ds-app"} 0)"),
              "ds-app waiting = 0（waiting+asyncWaiting）");

        const int connHelpCount = countMatches(text,
                                               std::string(R"(# HELP sqlconduit_pool_connections )"));
        check(connHelpCount == 1,
              "sqlconduit_pool_connections HELP 行只出现一次（无重复）");
    }
    ...
    std::cout << "== M3 指标导出：Prometheus 文本格式 — 慢 SQL + histogram ==\n";
    {
        ...
        check(contains(text, R"(# TYPE sqlconduit_slow_sql_count counter)"),
              "sqlconduit_slow_sql_count 类型为 counter");
        ...
        check(contains(text, R"(sqlconduit_slow_sql_count{data_source="ds-app",fingerprint="12345"} 7)"),
              "slow_sql_count = 7");
        check(contains(text, R"(sqlconduit_slow_sql_count{data_source="ds-app",fingerprint="67890"} 2)"),
              "第二条 slow_sql_count = 2");
        check(contains(text, "# TYPE sqlconduit_slow_sql_duration_seconds histogram"),
              "duration 使用标准 Prometheus histogram family");
        check(contains(
                  text, R"(sqlconduit_slow_sql_duration_seconds_count{data_source="ds-app",fingerprint="12345"} 7)"),
              "histogram family 包含 _count 样本");

        check(contains(
                  text,
                  R"(sqlconduit_slow_sql_duration_seconds_bucket{data_source="ds-app",fingerprint="12345",le="0.01"} 0)"),
              "histogram bucket le=0.01 cumulative = 0");
        check(contains(
                  text,
                  R"(sqlconduit_slow_sql_duration_seconds_bucket{data_source="ds-app",fingerprint="12345",le="0.1"} 7)"),
              "histogram bucket le=0.1 cumulative = 7");
        check(contains(
                  text,
                  R"(sqlconduit_slow_sql_duration_seconds_bucket{data_source="ds-app",fingerprint="12345",le="+Inf"} 7)"),
    ...
    std::cout << "== M3 指标导出：自定义 prefix ==\n";
    {
        ...
        check(!contains(text, "sqlconduit_"),
              "prefix=myapp 时输出不应出现 sqlconduit_ 前缀");
    }
