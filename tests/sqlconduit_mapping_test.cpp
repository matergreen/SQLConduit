...
int main() {
    ...
    std::cout << "== M1. 基本映射：全类型往返 ==\n";
    {
        ...
        const auto r = queryAs<User>(g_client, "SELECT * FROM users");
        ...
        const auto one = queryOneAs<User>(g_client, "SELECT * FROM users WHERE id = ?",
    ...
    std::cout << "== M2/M3. NULL 语义 ==\n";
    {
        ...
        const auto r = queryAs<User>(g_client, "SELECT 1");
        ...
        gRows = {bad};
        const auto r2 = queryAs<User>(g_client, "SELECT 2");
    ...
    std::cout << "== M4/M5. 列匹配：缺列默认跳过、多余列可配 ==\n";
    {
        ...
        gRows = {missing};
        const auto r = queryAs<User>(g_client, "SELECT 3");
        ...
        const auto strict = queryAs<StrictMissingUser>(g_client, "SELECT 3b");
        ...
        gRows = {extra};
        check(queryAs<User>(g_client, "SELECT 4").status.ok(),
              "多余列默认忽略（兼容 SELECT * / 联表）");
        check(!queryAs<StrictUser>(g_client, "SELECT 4b").status.ok(),
    ...
    std::cout << "== M6. 类型不符矩阵（严格模式全部报错）==\n";
    {
        ...
        check(queryAs<FlagRow>(g_client, "SELECT f1").status.ok(), "bool ← bool 通过");

        gRows = {RowData{{"active", common::Value(std::int64_t(1))}}};
        const auto b1 = queryAs<FlagRow>(g_client, "SELECT f2");
        ...
        check(queryAs<SmallRow>(g_client, "SELECT f3").status.ok(), "int32 ← int64 范围内通过");

        gRows = {RowData{{"qty", common::Value(std::int64_t(5000000000LL))}}};
        const auto o1 = queryAs<SmallRow>(g_client, "SELECT f4");
        ...
        const auto d1 = queryAs<StrictAmountRow>(g_client, "SELECT f5");
        ...
        check(!queryAs<StrRow>(g_client, "SELECT f6").status.ok(), "string ← Blob 报错");

        gRows = {RowData{{"ts", common::Value(std::string("2026-09-07 10:00:00"))}}};
        check(!queryAs<PlainTsRow>(g_client, "SELECT f7").status.ok(),
    ...
    {
        gRows = {RowData{{"amount", common::Value(common::Decimal{"12.50"})}}};
        const auto r = queryAs<LossyAmountRow>(g_client, "SELECT l1");
        check(r.status.ok() && r.items.size() == 1 && r.items[0].amount == 12.5,
              "Lossy 下 double ← Decimal 放行");

        gRows = {RowData{{"ts", common::Value(std::string("2026-09-07 10:00:00"))}}};
        const auto r2 = queryAs<TextualTsRow>(g_client, "SELECT l2");
    ...
    std::cout << "== M9. 自定义 ValueConverter ==\n";
    {
        ...
        const auto r = queryAs<RefRow>(g_client, "SELECT c1");
        ...
        check(!queryAs<RefRow>(g_client, "SELECT c2").status.ok(), "自定义转换器可拒绝不符类型");
    }
    ...
    std::cout << "== M11b. 标识符引号按方言生成 ==\n";
    {
        ...
        const std::string upd = mapping::updateSql<User>("users", common::util::Dialect::MySQL);
        check(upd.rfind("UPDATE `users` SET", 0) == 0, "MySQL 方言：UPDATE 表名用反引号");
    ...
    std::cout << "== M12. insertAs 生成键回填 ==\n";
    {
        ...
        const auto r = insertAs<User>(g_client, "users", u);
        ...
        User u2;
        u2.name = "bob";
        const auto r2 = insertAs<User>(g_client, "users", u2);
    ...
    std::cout << "== M13/M14. 无主键拒绝 / 批量 ==\n";
    {
        NoPkRow np;
        np.name = "x";
        const auto r = updateAs<NoPkRow>(g_client, "t", np);
        ...
        batch[0].name = "a";
        batch[1].name = "b";
        const auto br = insertBatchAs<User>(g_client, "users", batch);
    ...
    std::cout << "== M15. 流式 queryEachAs ==\n";
    {
        ...
        std::uint64_t rows = 0;
        const auto st = queryEachAs<User>(g_client, "SELECT stream", common::Params{},
                                          [&](User &&u) {
        ...
        int seen = 0;
        const auto st2 = queryEachAs<User>(g_client, "SELECT stream2", common::Params{},
        ...
        const auto st3 = queryEachAs<User>(g_client, "SELECT stream3", common::Params{},
    ...
    std::cout << "== M17. 事务内映射（Session 形态）==\n";
    {
        ...
        const auto st = g_client.transaction([&](core::Session &s) {
    ...
    std::cout << "== M18. 查询缓存：只缓存原始 ResultSet，命中后仍映射 ==\n";
    {
        ...
        const auto r1 = queryAs<User>(g_client, "SELECT cached");
        const int mid = gQueryCalls.load();
        const auto r2 = queryAs<User>(g_client, "SELECT cached");
    ...
    std::cout << "== M19. 脱敏顺序：映射在 afterExecution 之后 ==\n";
    {
        ...
        const auto r = queryAs<User>(g_client, "SELECT masked");
    ...
    std::cout << "== M20. Client future 异步查询后映射 ==\n";
    {
        gRows = {
        ...
        common::Params p;
        auto raw1 = g_client.queryAsync("SELECT async", p).get();
        EntityResult<User> o1;
        o1.status = raw1.status;
        if (o1.status.ok()) o1.status = mapping::fromRows<User>(raw1.rows, o1.items);
        check(o1.status.ok() && o1.items.size() == 2, "future 查询映射 2 行");
        check(o1.items.size() == 2 && o1.items[0].name == "async", "future 映射内容正确");

        common::Params p2;
        auto raw2 = g_client.queryAsync("SELECT async2", p2).get();
        EntityResult<User> o2;
        o2.status = raw2.status;
        if (o2.status.ok()) o2.status = mapping::fromRows<User>(raw2.rows, o2.items);
        check(o2.status.ok() && o2.items.size() == 2, "第二次 future 映射结果一致");

        common::Params p3;
        auto raw3 = g_client.queryAsync("SELECT async3", p3).get();
        EntityResult<User> o3;
        o3.status = raw3.status;
        if (o3.status.ok()) o3.status = mapping::fromRows<User>(raw3.rows, o3.items);
        check(o3.items.size() == o1.items.size() &&
              o3.items[1].id == o1.items[1].id, "多次 future 映射同源同结果");
    }
    ...
    std::cout << "== M21/M22. 异步映射失败与失败后可用性 ==\n";
    {
        ...
        gRows = {bad};
        common::Params p;
        auto raw = g_client.queryAsync("SELECT bad", p).get();
        EntityResult<User> o;
        o.status = raw.status;
        if (o.status.ok()) o.status = mapping::fromRows<User>(raw.rows, o.items);
        ...
        const auto r2 = queryAs<User>(g_client, "SELECT after-fail");
        check(r2.status.ok() && r2.items.size() == 1,
    ...
    std::cout << "== M23. 一致性矩阵：queryAs 与手工映射等价 ==\n";
    {
        ...
        common::ResultSet rs;
        const auto st = g_client.query("SELECT same", common::Params{}, rs);
        std::vector<User> manual;
        const auto ms = mapping::fromRows<User>(rs, manual);
        const auto viaClient = queryAs<User>(g_client, "SELECT same", common::Params{});
        check(st.ok() && ms.ok() && viaClient.status.ok(), "两条路径均成功");
        check(manual.size() == viaClient.items.size() && manual.size() == 1, "结果行数一致");
        check(manual.size() == 1 && viaClient.items.size() == 1 &&
              manual[0].id == viaClient.items[0].id &&
              manual[0].balance.value == viaClient.items[0].balance.value,
              "字段级完全一致");
    }
