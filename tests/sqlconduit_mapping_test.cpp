...
int main()
{
    ...
    std::cout << "== M17. 事务内映射（Session 形态）==\n";
    {
        ...
        const auto st = SQLConduit::transaction([&](core::Session& s)
        {
    ...
    std::cout << "== M18. 查询缓存：只缓存原始 ResultSet，命中后仍映射 ==\n";
    {
        ...
        cf.cache = true;
        applyConfig(cf);
        check(SQLConduit::reload(g_configPath, std::chrono::milliseconds(500)).ok(), "热加载开启查询缓存");
        ...
        CfgFlags back;
        applyConfig(back);
        check(SQLConduit::reload(g_configPath, std::chrono::milliseconds(500)).ok(), "恢复基础配置");
    }
    ...
    std::cout << "== M19. 脱敏顺序：映射在 afterExecution 之后 ==\n";
    {
        ...
        applyConfig(cf);
        check(SQLConduit::reload(g_configPath, std::chrono::milliseconds(500)).ok(), "热加载开启拦截器");
        SQLConduit::addInterceptor(std::make_shared<MaskingInterceptor>());
...
#else
        check(true, "协程形态未启用（SQLCONDUIT_ENABLE_ASYNC_CORO=OFF），跳过");
#endif
    }
    ...
    std::cout << "== M23. 一致性矩阵：queryAs 与手工映射等价 ==\n";
    {
        ...
        common::ResultSet rs;
        const auto st = SQLConduit::query("SELECT same", common::Params{}, rs);
