...
namespace
{
    ...
    struct Fixture
    {
        ...
        ~Fixture()
        {
            if (initialized)
            {
                ...
                (void)sqlconduit::SQLConduit::execute("DROP TABLE IF EXISTS " + table, affected);
        ...
        void start()
        {
            ...
            file.close();
            requireOk(sqlconduit::SQLConduit::init(configPath), "init");
            initialized = true;
            std::int64_t affected = 0;
    ...
    void testTransactionsBatchCursorAndAsync(Fixture& f)
    {
        ...
        const auto rollback = sqlconduit::SQLConduit::transaction([&](sqlconduit::core::Session& session)
        {
            std::int64_t affected = 0;
        ...
        ResultSet rolled;
        requireOk(sqlconduit::SQLConduit::query("SELECT COUNT(*) n FROM " + f.table
        ...
        int callbacks = 0;
        requireOk(sqlconduit::SQLConduit::queryEach("SELECT id FROM " + f.table + " ORDER BY id", {},
        ...
        requireOk(sqlconduit::SQLConduit::openCursor("SELECT id FROM " + f.table + " ORDER BY id", {},
        ...
        auto future = sqlconduit::async::query("SELECT COUNT(*) n FROM " + f.table);
        ...
        std::int64_t affected = 0;
        const auto duplicate = sqlconduit::SQLConduit::execute(
            "INSERT INTO " + f.table + " (name,qty,unsigned_value,created_at) "
        ...
        require(sqlconduit::SQLConduit::poolStats(pool) && pool.borrowRequests > 0, "pool metrics empty");
        require(!sqlconduit::SQLConduit::slowSqlStats().empty(), "slow SQL metrics empty");
    }
    ...
    void testEntityMapping(Fixture& f)
    {
        ...
        auto ins = sqlconduit::insertAs<MapItem>(f.table, item);
        ...
        auto got = sqlconduit::queryAs<MapItem>(
        ...
        item.qty = 99;
        auto upd = sqlconduit::updateAs<MapItem>(f.table, item);
        require(upd.status.ok() && upd.affected == 1, "updateAs failed");
        auto got2 = sqlconduit::queryAs<MapItem>(
            "SELECT id,name,qty,unsigned_value FROM " + f.table + " WHERE id=?",
            Params{std::int64_t(id)});
        ...
        auto batch = sqlconduit::insertBatchAs<MapItem>(f.table, bv);
        ...
        auto each = sqlconduit::queryEachAs<MapItem>(
    ...
    void testRoutinesAndCall(Fixture& f)
    {
        ...
        require(sqlconduit::common::util::createRoutine(addFn).ok(), "createRoutine(function) failed");

        ResultSet rs;
        auto st = sqlconduit::common::util::callQuery(
            "SELECT sqlconduit_it_add(?,?)",
            Params{std::int64_t(3), std::int64_t(4)}, rs, sqlconduit::common::util::CallOptions{});
        ...
        require(sqlconduit::common::util::createRoutine(swapProc).ok(),
        ...
        {
            return sqlconduit::common::util::call(s, swapRef, io, ioRes,
        ...
        require(sqlconduit::common::util::createRoutine(multiProc).ok(),
                "createRoutine(procedure multi) failed");
        RoutineRef multiRef{"sqlconduit_it_multi", RoutineKind::Procedure};
        CallResult multiRes;
        auto mSt = sqlconduit::common::util::call(multiRef, CallParams{}, multiRes,
                                                  sqlconduit::common::util::CallOptions{});
        ...
        std::int64_t d = 0;
        requireOk(sqlconduit::SQLConduit::execute("DROP FUNCTION IF EXISTS sqlconduit_it_add", d), "drop function");
        requireOk(sqlconduit::SQLConduit::execute("DROP PROCEDURE IF EXISTS sqlconduit_it_swap", d), "drop proc swap");
        requireOk(sqlconduit::SQLConduit::execute("DROP PROCEDURE IF EXISTS sqlconduit_it_multi", d),
                  "drop proc multi");
    }
    ...
    {
        const std::string fn =
            "CREATE FUNCTION sqlconduit_it_aadd(a INT, b INT) RETURNS INT DETERMINISTIC RETURN a + b";
