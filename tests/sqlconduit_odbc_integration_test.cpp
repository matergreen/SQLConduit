...
namespace
{
    ...
    void testTransactionsPreparedBatchCursorAsync(Fixture& f)
    {
        ...
        const auto rolled = sqlconduit::SQLConduit::transaction([&](sqlconduit::core::Session& session)
        {
            std::int64_t affected = 0;
        ...
        const auto asyncRows = sqlconduit::async::query("SELECT COUNT(*) n FROM " + f.table).get();
    ...
    void testEntityMapping(Fixture& f)
    {
        ...
        auto ins = sqlconduit::insertAs<OdbcItem>(f.table, item);
        ...
        auto got = sqlconduit::queryAs<OdbcItem>(
        ...
        item.qty = 99;
        auto upd = sqlconduit::updateAs<OdbcItem>(f.table, item);
        require(upd.status.ok() && upd.affected == 1, "updateAs failed");
        auto got2 = sqlconduit::queryAs<OdbcItem>(
            "SELECT id,name,qty,amount,created_at FROM " + f.table + " WHERE id=?",
            Params{std::int64_t(id)});
        ...
        auto batch = sqlconduit::insertBatchAs<OdbcItem>(f.table, {b1, b2});
        ...
        auto each = sqlconduit::queryEachAs<OdbcItem>(
    ...
    void testScriptExecution(Fixture& f)
    {
        ...
        requireOk(sqlconduit::SQLConduit::execute("DROP PROCEDURE sqlconduit_it_script_proc", dropped),
    ...
    void testRoutinesAndCall(Fixture& f)
    {
        require(sqlconduit::common::util::createRoutine(
                    "CREATE FUNCTION sqlconduit_it_add(@a INT, @b INT) RETURNS INT AS BEGIN RETURN @a + @b END").ok(),
        ...
        auto rowsSt = sqlconduit::common::util::call(rowsRef, CallParams{}, rowsRes, rowsOpts);
        ...
        std::int64_t d = 0;
        requireOk(sqlconduit::SQLConduit::execute("DROP FUNCTION sqlconduit_it_add", d), "drop fn add");
        requireOk(sqlconduit::SQLConduit::execute("DROP PROCEDURE sqlconduit_it_rows", d), "drop procedure rows");
    }

    void testAsyncUtil(Fixture& f)
    {
        const std::string fn =
            "CREATE FUNCTION sqlconduit_it_aadd(@a INT, @b INT) RETURNS INT AS BEGIN RETURN @a + @b END";
        ...
        std::int64_t d = 0;
        requireOk(sqlconduit::SQLConduit::execute("DROP FUNCTION sqlconduit_it_aadd", d), "drop async function");
    }
}
