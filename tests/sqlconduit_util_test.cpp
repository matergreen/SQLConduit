...
int main() {
    ...
    std::cout << "== U21. makeCallPlan：OUT/INOUT 的 SQL 编排 ==\n";
    {
        ...
        check(util::makeCallPlan(proc, withOut, util::Dialect::MySQL, true, outPlan).ok() &&
              outPlan.callSql == "CALL `p`(?, @sqlconduit_out_1)" &&
              outPlan.fetchSql == "SELECT @sqlconduit_out_1 AS sqlconduit_out_1" &&
        ...
        check(util::makeCallPlan(proc, inOut, util::Dialect::MySQL, true, ioPlan).ok() &&
              ioPlan.preSql == "SET @sqlconduit_out_0 = ?" &&
              ioPlan.callSql == "CALL `p`(@sqlconduit_out_0)" &&
              ioPlan.fetchSql == "SELECT @sqlconduit_out_0 AS sqlconduit_out_0" &&
    ...
    std::cout << "== U24. Session 路径：OUT 回读 ==\n";
    {
        ...
        gSets = {{r1a, r1b}};
        RowData outRow{{"sqlconduit_out_1", common::Value(std::int64_t(42))}};
        gRows = {outRow};
    ...
    {
        resetCounters();
        RowData outRow{{"sqlconduit_out_0", common::Value(std::int64_t(11))}};
        gRows = {outRow};
        gSets.clear();
    ...
    std::cout << "== U27. OUT 回读的失败路径 ==\n";
    {
        ...
        check(util::readOutParams(noRow, {"sqlconduit_out_0"}, outs).code == ErrorCode::QueryError,
              "U27 回读无行 → QueryError");
